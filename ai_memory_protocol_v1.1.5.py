...
_PLATFORM_DEFAULT_DB = "/opt/claw/hermes/protocol/mark_memory.db"
DEFAULT_DB_PATH     = _MARK_MEMORY_DB_ENV or _PLATFORM_DEFAULT_DB

# Paths
_PROTOCOL_DIR = Path(__file__).parent.resolve() if "__file__" in dir() else Path("/opt/claw/hermes/protocol")
...
def _gc_audit(result: Dict[str, Any]):
    try:
        ...
        with open(AUDIT_PATH, "a") as f:
            f.write(json.dumps({"timestamp": _utcnow(), "result": result}, ensure_ascii=False) + "\n")
    except Exception as e:
        _gc_log(f"Audit write error: {e}")
